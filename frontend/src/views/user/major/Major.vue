    ...
    <a-row style="margin-top: 25px">
      <a-col :span="24">
        <a-tabs
          v-model="checkFlag"
          @change="tabChange">
          <a-tab-pane v-for="(item, index) in provinceList" :key="item" :tab="item"></a-tab-pane>
        </a-tabs>
      </a-col>
      <!--      <a-col :span="12">-->
      <!--        <div>-->
...
export default {
  ...
  methods: {
    ...
    selectSchoolRate (type, schoolName) {
      ...
      this.$get(`/cos/score-line-info/selectRecommendByMaior`, params).then((r) => {
        let data = r.data.data
        this.dataList = data
        if (this.dataList) {
          this.provinceList = data.map(item => item.name)
          this.currentTab = this.provinceList[0]
          this.page.total = data[0].list.length
    ...
    tabChange (checkFlag) {
      ...
      if (this.dataList) {
        ...
        if (result) {
          this.page.total = result[0].list.length
          this.currentDataList = result[0].list.slice((this.page.current - 1) * this.page.size, this.page.current * this.page.size)
          console.log(this.currentDataList.length)
          console.log(this.page)
        }
      }
    },
    queryProvinces () {
      this.$get('/cos/sys-school/selectRecommendByMaior').then((r) => {
        // 获取所有省份
